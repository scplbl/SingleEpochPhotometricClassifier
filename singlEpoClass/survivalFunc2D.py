...
def find_percentile(my_dir, file_dir, filter1, filter2, flux_filter1,
                    flux_filter2, flux_filter1_err,
                    flux_filter2_err, z):

    all_percentile = []
    data_point = (flux_filter1, flux_filter2)
    ...
    Ia_percentile = kde3d(mc_dict['type_Ia']['flux'][filter1],
                          mc_dict['type_Ia']['flux'][filter2], data_point)

    Ibc_percentile = kde3d(mc_dict['type_Ibc']['flux'][filter1],
                           mc_dict['type_Ibc']['flux'][filter2], data_point)

    II_percentile = kde3d(mc_dict['type_II']['flux'][filter1],
                          mc_dict['type_II']['flux'][filter2], data_point)
