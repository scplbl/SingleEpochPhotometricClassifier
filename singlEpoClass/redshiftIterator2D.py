...
from randomForest2D import obtain_proba
from survivalFunc2D import find_percentile


def iterator(type, my_dir, file_dir, filter1, filter2,
             flux_filter1, flux_filter2, flux_filter1_err,
             flux_filter2_err, photo_z_type=None,
    ...
    if type == 'RF':
        for i in range(len(files[0])):
            RF = obtain_proba(
              ...
              flux_filter2, flux_filter1_err, flux_filter2_err, z[i])[0]
            quant.append(RF)
...
def save_arrays(my_dir, file_dir, filter1, filter2, flux_filter1,
                flux_filter2, flux_filter1_err,
                flux_filter2_err, outdir):

    rf, rf_z = iterator('RF', my_dir, file_dir, filter1, filter2,
                        flux_filter1, flux_filter2,
                        flux_filter1_err, flux_filter2_err)
