...
def get_data(my_dir, file_dir, filter1, filter2, flux_filter1_err,
             flux_filter2_err, z):
    ...
    data = [all_filter1, all_filter2]
    data = zip(*data)
    all_sources = pd.DataFrame(data, columns=[filter1, filter2],
                               index=range(3*n))
...
def obtain_proba(my_dir, file_dir, filter1, filter2, flux_filter1,
                 flux_filter2, flux_filter1_err,
                 flux_filter2_err, z):

    X, Y = get_data(my_dir, file_dir, filter1, filter2,
                    flux_filter1_err, flux_filter2_err, z)
    ...
    rfc = clf.fit(X, Y)
    proba = rfc.predict_proba([flux_filter1, flux_filter2])
    return(proba)
